...
class Dataset(torch.utils.data.Dataset): #default setting for dataset
    def __init__(self, split):
        self.dataset = load_dataset("json",
                                    data_files='/Users/huangshixun/Desktop/Transformer/output.json',
                                    split=split)
    ...
    def __getitem__(self, i):
        ...
        category = self.label_encoder.transform([self.dataset[i]['category']])[0]
...
def collate_fn(data):# method for dealing data
...
class Model(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self.fc = torch.nn.Linear(768, len(set(dataset.dataset['category']))) #the regression model depends on the number of text classifications you want to implement
...
optimizer = AdamW(model.parameters(), lr=5e-4) #model set
...
for i, (input_ids, attention_mask, token_type_ids, categories) in enumerate(
    ...
    if i % 15 == 0: #display result of loss info and accuracy each 15 steps
        out = out.argmax(dim=-1)
        accuracy = (out == categories).sum().item() / len(categories)
        print(f"Step: {i}, Loss: {loss.item()}, Accuracy: {accuracy}")

    if i == 1000: # stop running when the 1000steps reach
        break 
